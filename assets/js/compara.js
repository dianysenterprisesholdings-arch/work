...
(() => {
  ...
  if (!ids.length) {
    out.innerHTML = `<div class="ec-empty">
      <p>Nu ai salvat încă niciun apartament.</p>
  ...
  fetch('../assets/data/unitati.json').then(r => r.json()).then(d => {
    const F = {};
    ...
    if (!gasite.length) { out.innerHTML = '<div class="ec-empty"><p>Apartamentele din link nu au fost găsite.</p></div>'; return; }
    ...
    const cap = gasite.map(u =>
      `<th><a href="../apartamente-iasi/${u[F.id].toLowerCase()}/">${u[F.id]}</a></th>`).join('');
    ...
    out.innerHTML = `<div class="ec-cmp">
      <table>
        ...
        <tbody>
          ${rand('Preț', u => `<span class="${u[F.pret] === min('pret') ? 'best' : ''}">${euro(u[F.pret])}</span>`)}
          ${rand('Preț pe m²', u => `<span class="${ppm(u) === ppmMin ? 'best' : ''}">${ppm(u)} €/m²</span>`)}
          ${rand('Suprafață utilă', u => `<span class="${u[F.su] === max('su') ? 'best' : ''}">${mp(u[F.su])}</span>`)}
          ...
          ${rand('Boxă', u => u[F.boxa] === 'da' ? 'Disponibilă' : '—')}
          ${rand('Parcare subterană', u => u[F.parcare] === 'da' ? 'Da' : 'Supraterană')}
          ${rand('Etapa', u => u[F.etapa])}
          ${rand('Stare', u => `<span class="ec-tag ec-tag--${u[F.status]}">${ST[u[F.status]]}</span>`)}
          ${rand('', u => `<a class="ec-btn ec-btn--out" href="../apartamente-iasi/${u[F.id].toLowerCase()}/">Vezi</a>`)}
        </tbody>
      </table>
    </div>
    <p class="ec-calc__note">Valorile marcate cu verde sunt cele mai bune din comparație.</p>`;
  }).catch(() => {
    out.innerHTML = '<div class="ec-empty"><p>Datele nu au putut fi încărcate.</p></div>';
  });
